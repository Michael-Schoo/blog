...
The last week I was required to do some tasks with Object Oriented Programming [(more in other post)](/p/object-oriented-programming). While doing it I decided to test my code, on making the JK Flip-flop. But it wasn't working because of a `referenced before assignment` error, a fancy way to mention recursion issues. Having errors is one of a developer's most frustrating/common part, so I was keen to try to fix it.
